import Functions.{getProductsByMonthInterval, parseDate}
...
object Main extends App {
  ...
  def productsAvailable: List[String] = DB readOnly { implicit session =>
    sql"select distinct(product_name) from dott_db_test.products"
      .map(rs => rs.string("product_name")).list.apply()
  }
  ...
  val productName: String = readLine("Enter the name of the product to be searched for: \n")
  println("product's name -> " + productName)

  val dataInterval: (LocalDateTime, LocalDateTime) = parseDate(readLine("Enter the date range for searching: example -> \"2018-01-01 00:00:00\" \"2019-01-01 00:00:00\" \n").split(" (?=(?:\"[^\"]*\"|[^\"])*$)").toList)
  println("date informed for search -> " + dataInterval)

  val monthInterval: List[String] = readLine("Enter how this range will be displayed in months:  example -> (“1-3”, “4-6”, “7-12”, “>12”) \n").split("[,()]").toList.drop(1)
